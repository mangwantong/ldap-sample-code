...
/**
 * <p>
 * Provides an example of the use of the {@code MatchedValuesRequestControl}.
 * The matched values request control is useful in cases where an application
 * wants to the server to return only certain values of a multi-valued
 * attribute. The values to return match the {@literal "matched values filter"}.
 * <p>
 * The user provides the options required to connect to the directory server, a
 * base object, a filter, a search scope, and a list of requested attributes
 * plus a matched values filter specified by the {@code --matchedValuesFilter}
 * command line argument. The demonstration displays the entry returned as the
 * result of a search request with the matched values request control attached.
 * <p>
...
@Since("Dec 7, 2011")
@CodeVersion("1.0")
public final class MatchedValuesRequestControlExample extends
		LDAPCommandLineTool {

	/**
	 * The long identifier of the command line argument whose parameter is the
	 * matched values filter, that is, the filter that is used to match the values
	 * of an attribute to return.
	 */
	...
	/**
	 * The description of the tool; this description is used in displaying command
	 * line help options and for other purposes.
	 */
	...
	/**
	 * The name of the tool; this name is used in displaying command line help
	...
	/**
	 * Launch the AuthDemo application. Takes the following command line arguments
	 * in addition to the standard ones:<blockquote>
	 * 
	 * <pre>
	...
	public static void main(final String... args) {
		...
		final MatchedValuesRequestControlExample matchedValuesRequestControlExample = MatchedValuesRequestControlExample
				.newMatchedValuesRequestControlExample(outStream, errStream,
						loggingFormatter);
		final ResultCode resultCode = matchedValuesRequestControlExample
				.runTool(args);
		final ToolCompletedProcessing completedProcessing = new BasicToolCompletedProcessing(
				matchedValuesRequestControlExample, resultCode);
	...
	private static MatchedValuesRequestControlExample newMatchedValuesRequestControlExample(
		...
		return new MatchedValuesRequestControlExample(outStream, errStream,
				loggingFormatter);
	...
	static {
		...
		THE_TOOL_DESCRIPTION = "Provides an example of the use of the MatchedValuesRequestControl. "
	...
	public void addNonLDAPArguments(final ArgumentParser argumentParser)
		...
		this.commandLineOptions = CommandLineOptions
				.newCommandLineOptions(argumentParser);

		/*
		 * The command line argument which specifies the filter to use to match
		 * attribute values. This command line argument is required, and can occur
		 * exactly once.
		 */
		final Character shortIdentifier = null;
		final String longIdentifier = MatchedValuesRequestControlExample.ARG_NAME_MATCHED_VALUES_FILTER;
		...
		final FilterArgument filterArgument = new FilterArgument(shortIdentifier,
	...
	public ResultCode doToolProcessing() {
		...
		try {
			...
			out(this.loggingFormatter.format(logRecord));

			/*
			 * Determine whether the matched values request control is supported by
			 * the server.
			 */
			final SupportedFeature supportedFeature = SupportedFeature
					.newSupportedFeature(ldapConnection);
			...
			logRecord = new LogRecord(Level.INFO,
					"MatchedValuesRequestControl is supported.");
			out(this.loggingFormatter.format(logRecord));

			/*
			 * Handles unsolicited notifications. An unsolicited notification is an
			 * LDAPMessage sent from the server to the client that is not in response
			 * to any LDAPMessage received by the server. It is used to signal an
			 * extraordinary condition in the server or in the LDAP session between
			 * the client and the server. The notification is of an advisory nature,
			 * and the server will not expect any response to be returned from the
			 * client.
			 */
			final UnsolicitedNotificationHandler unsolicitedNotificationHandler = new UnsolicitedNotificationHandler() {

				@Override
				public void handleUnsolicitedNotification(
						final LDAPConnection ldapConnection,
						final ExtendedResult extendedResult) {
					final String msg = String.format("recd unsolicited notification: %s",
							extendedResult.getDiagnosticMessage());
					final LogRecord logRecord = new LogRecord(Level.WARNING, msg);
					out(MatchedValuesRequestControlExample.this.loggingFormatter
							.format(logRecord));
				}

			};

			final LDAPConnectionOptions ldapConnectionOptions = this.commandLineOptions
			...
			/*
			 * Construct the search request using parameters from the --baseObject,
			 * --scope, --filter, and --attribute command line arguments.
			 */
			final String baseObject = this.commandLineOptions.getBaseObject();
			final SearchScope scope = this.commandLineOptions.getSearchScope();
			final Filter filter = this.commandLineOptions.getFilter();
			final String[] requestedAttributes = this.commandLineOptions
					.getRequestedAttributes();
			final SearchRequest searchRequest = new SearchRequest(baseObject, scope,
					filter, requestedAttributes);

			final ArgumentParser argumentParser = this.commandLineOptions
					.getArgumentParser();
			final FilterArgument filterArgument = (FilterArgument) argumentParser
			...
			final MatchedValuesFilter matchedValuesFilter = MatchedValuesFilter
					.create(filt);
			final MatchedValuesRequestControl control = new MatchedValuesRequestControl(
					matchedValuesFilter);
			...
			if ((searchResult != null) && (searchResult.getEntryCount() > 0)) {
				for (final SearchResultEntry entry : searchResult.getSearchEntries()) {
					...
					final String msg = this.loggingFormatter.format(logRecord);
					out(msg);
				}
			}
			...
			logRecord = new LogRecord(Level.INFO,
					"MatchedValuesRequestControl not supported.");
			err(this.loggingFormatter.format(logRecord));
	...
	/**
	 * Prepares {@code MatchedValuesRequestControlExample} for use by a client -
